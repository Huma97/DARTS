...
def get_cifar():
    ...
    Y_train = keras.utils.np_utils.to_categorical(train_data.targets, nb_classes).astype('float32')
    Y_test  = keras.utils.np_utils.to_categorical(test_data.targets, nb_classes).astype('float32')

    Dataset = namedtuple('Dataset',['X','Y','y','nb_classes'])
    trn = Dataset(train_data.data, Y_train, np.asarray(train_data.targets), nb_classes)
    tst = Dataset(test_data.data , Y_test, np.asarray(test_data.targets), nb_classes)
